...
bash ~/src/claude-skills/write-as-bro/install.sh            # personal: ~/.claude
bash ~/src/claude-skills/write-as-bro/install.sh --project  # or just this repo
```

The installer symlinks the skill into `~/.claude/skills/write-as-bro` and adds
`@~/.claude/skills/write-as-bro/ALWAYS.md` to `CLAUDE.md`, so the dash ban is in context
on every turn instead of only when the skill loads. Re-running it changes
nothing.

Invoke the full skill with `/write-as-bro`, or let it trigger on its own when I say a
draft sounds like AI.

## The checker

```bash
bash ~/.claude/skills/write-as-bro/scripts/ai-tells.sh README.md docs/
```
...
```bash
bash ~/.claude/skills/write-as-bro/scripts/ai-tells.sh $(git diff --cached --name-only --diff-filter=ACM -- '*.md')
```
...
Different jobs. `me` decides whose voice it is, first person, I built this,
not a tutorial teaching me. `write-as-bro` decides whether the prose sounds like
a human wrote it. Install both and they stack: my voice, and it reads like mine.
